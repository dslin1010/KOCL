...
static int crypt_scatterlist2(struct ecryptfs_crypt_stat *crypt_stat,
			       ...
			       unsigned char *iv, int op)
{
	...
	rc = op == ENCRYPT ? crypto_blkcipher_encrypt_iv(&desc, dest_sg, src_sg, size):
					     crypto_blkcipher_decrypt_iv(&desc, dest_sg, src_sg, size);;
	mutex_unlock(&crypt_stat->cs_tfm_mutex);
out:
	return rc;
}
			...
			unsigned long extent_offset, int op)
{
	...
	rc = crypt_scatterlist2(crypt_stat, &dst_sg, &src_sg, extent_size,
			       extent_iv, op);
	if (rc < 0) {
...
int ecryptfs_encrypt_pages2(struct page **pgs, unsigned int nr_pages)
{
	...
	do_gettimeofday(&t0);
	//rc = crypt_scatterlist(cst, sgd, sgs, PAGE_SIZE*nr_pages, fake_iv, ENCRYPT);
	rc = crypt_scatterlist2(cst, sgd, sgs, PAGE_SIZE*nr_pages, fake_iv, ENCRYPT);
	rc = 0;
	do_gettimeofday(&t1);
		enc = 1000000*(t1.tv_sec-t0.tv_sec) + 
			((int)(t1.tv_usec) - (int)(t0.tv_usec));
    printk("  enc time: %6ld us \n", enc);	
   ...
   // printk("[g-ecryptfs] decrypt pages %u\n", nr_pages); 
    ...
    for (i=0; i<nr_pages; i++) {
    	...
    	offset = ecryptfs_lower_header_size(cst)
    		+ ((loff_t)(pgs[i]->index)<<PAGE_SHIFT);
    	virt = kmap(pgs[i]);
    	rc = ecryptfs_read_lower(virt, offset, PAGE_SIZE, ind);//把disk資料讀到pgs[i]
	...
	sg_set_page(sgs+i, pgs[i], PAGE_SIZE, 0);//把scatterlist 指標指向page cache 的page
    }

    /* no IV for our implementation */
    rc = crypt_scatterlist2(cst, sgs, sgs, nr_pages*PAGE_SIZE, iv, DECRYPT);//開始做解密
out:
    if (sgs)
	kfree(sgs);
