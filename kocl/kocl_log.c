...
#ifdef __KOCL_LOG_LEVEL__
int kocl_log_level = __KOCL_LOG_LEVEL__;
#else
int kocl_log_level = KOCL_LOG_ALERT;
#endif

void
kocl_generic_log(int level, const char *module, const char *filename,
		 int lineno, const char *func, const char *fmt, ...)
{
    va_list args;
    
    if (level < kocl_log_level)
	return;
...
EXPORT_SYMBOL_GPL(kocl_generic_log);
EXPORT_SYMBOL_GPL(kocl_log_level);
